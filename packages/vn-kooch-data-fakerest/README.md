# FakeRest Data Provider For vn-kooch-react-admin

Client-side data provider for [vn-kooch-react-admin](https://github.com/marmelab/vn-kooch-react-admin), the frontend framework for building admin applications on top of REST/GraphQL services.
...
All operations carried out in vn-kooch-react-admin are local to the browser, and last only for the current browser session. A browser refresh erases all modifications.
...
```sh
npm install --save vn-kooch-data-fakerest
```
...
import fakeDataProvider from 'vn-kooch-data-fakerest';
