...
- [The `<Admin>` Component](#the-admin-component)
    ...
    - [Using vn-kooch-react-admin without `<Admin>` and `<Resource>`](#using-vn-kooch-react-admin-without-admin-and-resource)
...
By default, the homepage of an admin app is the `list` of the first child `<Resource>`. But you can also specify a custom component instead. To fit in the general design, use Material UI's `<Card>` component, and vn-kooch-react-admin's `<Title>` component to set the title in the AppBar:
...
You can customize this page to use the component of your choice by passing it as the `catchAll` prop. To fit in the general design, use Material UI's `<Card>` component, and vn-kooch-react-admin's `<Title>` component:
...
**Tip**: If your custom `catchAll` component contains react-router `<Route>` components, this allows you to register new routes displayed within the vn-kooch-react-admin layout easily. Note that these routes will match *after* all the vn-kooch-react-admin resource routes have been tested. To add custom routes *before* the vn-kooch-react-admin ones, and therefore override the default resource routes, use the [`customRoutes` prop](#customroutes) instead.

## `menu`

**Tip**: This prop is deprecated. To override the menu component, use a [custom layout](#applayout) instead.

vn-kooch-react-admin uses the list of `<Resource>` components passed as children of `<Admin>` to build a menu to each resource with a `list` component.
...
If you want to deeply customize the app header, the menu, or the notifications, the best way is to provide a custom layout component. It must contain a `{children}` placeholder, where vn-kooch-react-admin will render the resources. If you use material UI fields and inputs, it should contain a `<MuiThemeProvider>` element. And finally, if you want to show the spinner in the app header when the app fetches data in the background, the Layout should connect to the redux store.

Use the [default layout](https://github.com/marmelab/vn-kooch-react-admin/blob/master/packages/vn-kooch-ui-materialui/src/layout/Layout.js) as a starting point, and check [the Theming documentation](./Theming.md#using-a-custom-layout) for examples.
...
```jsx
{
    admin: { /*...*/ }, // used by vn-kooch-react-admin
...
**Tip**: Your custom pages take precedence over vn-kooch-react-admin's own routes. That means that `customRoutes` lets you override any route you want! If you want to add routes *after* all the vn-kooch-react-admin routes, use the [`catchAll` prop](#catchall) instead.

**Tip**: To look like other vn-kooch-react-admin pages, your custom pages should have the following structure:
...
If you want to customize the Login page, or switch to another authentication strategy than a username/password form, pass a component of your own as the `loginPage` prop. vn-kooch-react-admin will display this component whenever the `/login` route is called.
...
## Using vn-kooch-react-admin without `<Admin>` and `<Resource>`
